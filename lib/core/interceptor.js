...
/**
 * The inbound HTTP path this request arrived on, for `paths` scoping (see policy/evaluator.js).
 *
 * `req.http.req` is Express's request object, and it carries the path of the *outermost* HTTP
 * request even when a runtime mounted elsewhere calls the service internally rather than over HTTP.
 * That is exactly what makes this usable: a tool call handled at `/mcp` which then queries the same
 * entity the UI reads still reports `/mcp` here, so the two can be told apart without either of
 * them saying which it is.
 *
 * `originalUrl` first because it survives Express's mount-prefix stripping, then `path` (no query
 * string), then `url` as a last resort. Query strings are cut off — a scope is about the door, and
 * leaving `?$top=1` on the end would break every prefix comparison.
 */
function resolvePath(req) {
  const httpReq = req && req.http && req.http.req;
  if (!httpReq) return undefined;

  const raw = httpReq.originalUrl || httpReq.path || httpReq.url;
