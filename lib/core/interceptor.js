 ...
 */
function resolveMaskValues(fieldsToMask, elements, maskOptions = {}) {
  const { value = MASK_VALUE, typeSafe = true } = maskOptions;
  const values = {};
  for (const field of fieldsToMask) {
    const element = elements && elements[field];
    const fits = !typeSafe || !element || PLACEHOLDER_SAFE_TYPES.has(element.type);
    values[field] = fits ? value : null;
  }
  return values;
}
 ...
 */
function applyMask(results, fieldsToMask, elements, maskOptions) {
  const maskValues = resolveMaskValues(fieldsToMask, elements, maskOptions);
    ...
    for (const { name, targetEntity, targetElements, targetDef } of associations) {
      ...
      if (decision.mode === 'enforce') {
        if (decision.fieldsToMask.length > 0) {
          applyMask(navValue, decision.fieldsToMask, targetElements, maskOptions);
        }
  ...
  srv.after('*', (results, req) => {
    ...
    if (policyDefinition) {
      ...
      if (decision.mode === 'enforce' && decision.fieldsToMask.length > 0) {
        applyMask(results, decision.fieldsToMask, req.target && req.target.elements, maskOptions);
      }
