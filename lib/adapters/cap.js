...
const { lintPolicy, lintIdentityScoping, lintPathScoping, formatLintReport } = require('../policy/lint');
 ...
 * @param {boolean|{strict?: boolean}} [lint]
 */
// CAP pseudo-roles that every caller past authentication already satisfies. `@requires:
// 'authenticated-user'` is the most-written authorization annotation there is, and against an agent
// that authenticated perfectly well it closes nothing — so counting its mere presence as "this door
// is locked" would keep the path-scoping warning silent in exactly the common case it exists for.
const OPEN_TO_ANY_CALLER = new Set(['any', 'authenticated-user']);

const asList = (value) => (Array.isArray(value) ? value : [value]);

function requiresNarrows(requires) {
  if (requires === undefined) return false;
  return asList(requires).some((role) => typeof role === 'string' && !OPEN_TO_ANY_CALLER.has(role));
}

/**
 * Whether `@restrict` narrows reading to something an arbitrary caller isn't. A grant covering READ
 * (or '*') and given `to` a pseudo-role leaves reading open; anything else is a real restriction.
 */
function restrictNarrows(restrict) {
  if (restrict === undefined) return false;

  const grants = asList(restrict).filter((g) => g && typeof g === 'object');
  if (grants.length === 0) return false;

  const readGrants = grants.filter((g) =>
    asList(g.grant).some((op) => op === '*' || String(op).toUpperCase() === 'READ')
  );
  // No grant mentions reading at all: reading isn't granted to anyone, which is narrower still.
  if (readGrants.length === 0) return true;

  return readGrants.every((g) => requiresNarrows(g.to));
}

function carriesAuth(definition) {
  if (!definition) return false;
  return requiresNarrows(definition['@requires']) || restrictNarrows(definition['@restrict']);
}

/**
 * Collects `{entityName: {service, entity}}` — whether an authorization annotation was found on
 * each served entity's service and on the entity itself. The CAP-shaped input lintPathScoping()
 * needs, which is why the check itself can stay in lib/policy knowing nothing about @sap/cds.
 */
function collectAuthorization(cds) {
  const byEntity = {};
  for (const srv of Object.values((cds && cds.services) || {})) {
    const serviceAuth = carriesAuth(srv && srv.definition);
    for (const [name, entityDef] of Object.entries((srv && srv.entities) || {})) {
      if (!entityDef) continue;
      byEntity[entityDef.name || name] = { service: serviceAuth, entity: carriesAuth(entityDef) };
    }
  }
  return byEntity;
}
...
/**
 * Reports the two scoping choices whose weak half config alone cannot supply — identity scoping
 * resting on authentication that cannot carry it, and path scoping leaving the other doors open
 * (see lint.js#lintIdentityScoping and #lintPathScoping).
 *
 * Unlike runPolicyLint(), neither is gated on `lint` being switched on. Every other finding is
 * about a payload being shaped wrongly, which the person who asked for the lint is the one who
 * wants to hear about. These are about the policy silently not applying at all, and someone who
 * never enabled the lint is exactly who needs telling. `lint: { strict: true }` still upgrades
 * them to a refusal, which is how a production pipeline can make sure they were acted on.
 *
 * @param {object} cds read for cds.env.requires.auth.kind and the served model's annotations
 * @param {object} policyDefinition merged policy
 * @param {boolean|{strict?: boolean}} [lint]
 */
function reportScopingRisks(cds, policyDefinition, lint) {
  const auth = cds && cds.env && cds.env.requires && cds.env.requires.auth;
  const warnings = [
    lintIdentityScoping(policyDefinition, auth && auth.kind),
    lintPathScoping(policyDefinition, collectAuthorization(cds))
  ].filter(Boolean);
  ...
  if (typeof lint === 'object' && lint !== null && lint.strict) {
    throw new Error(
      `[cap-mcp-guard] refusing to start with lint.strict set:\n${warnings.map((w) => `  - ${w}`).join('\n')}`
    );
  }
}
...
function registerCapMcpGuard(cds, options = {}) {
  ...
  cds.on('served', () => {
    ...
    reportScopingRisks(cds, policyDefinition, lint);
