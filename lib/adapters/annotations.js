...
function readPseudonymizeAnnotation(element, field) {
  ...
  for (const [key, value] of Object.entries(element)) {
    if (key.startsWith(`${PSEUDONYMIZE_KEY}.`)) {
      flattened[key.slice(PSEUDONYMIZE_KEY.length + 1)] = value;
    }
  }
  if (Object.keys(flattened).length > 0) return { field, ...flattened };
 ...
 *
 * CAP flattens annotation paths onto elements/entities as dotted keys, so
 * `salary : Decimal @mcp.policy.mask;` shows up as `element['@mcp.policy.mask'] === true`.
 * `pseudonymize` has several shapes and its own reader — see readPseudonymizeAnnotation().
 *
