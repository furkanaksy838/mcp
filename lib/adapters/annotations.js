 ...
 *
 * CAP flattens nested annotation paths onto elements/entities as dotted keys, so
 * `salary : Decimal @mcp.policy.mask;` shows up as `element['@mcp.policy.mask'] === true`,
 * and `pseudonymize` as either a bare string (`@mcp.policy.pseudonymize: 'iban'`) or an
 * object (`@mcp.policy.pseudonymize: {type: 'custom', value: '...'}`).
 *
...
function readEntityAnnotations(entityDef) {
  ...
  for (const [field, element] of Object.entries(entityDef.elements || {})) {
    ...
    if (element[`${ANNOTATION_PREFIX}mask`] === true) {
      mask.push(field);
    }

    const pseudo = element[`${ANNOTATION_PREFIX}pseudonymize`];
    if (typeof pseudo === 'string') {
      pseudonymize.push({ field, type: pseudo });
    } else if (pseudo && typeof pseudo === 'object' && !Array.isArray(pseudo)) {
      pseudonymize.push({ field, ...pseudo });
    }
  }
