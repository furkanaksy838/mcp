...
// What to do with a request the guard cannot place: `paths` is set, but this request carries no
// inbound path to compare against. 'mask' (the default) applies the policy — see
// policy/evaluator.js#pathInScope for why the ambiguous case leans that way.
const WHEN_PATH_UNKNOWN = ['mask', 'pass'];
...
const MASK_TYPES = ['full', 'partial', 'email'];

/**
 * Normalizes a `"mask"` array into the two shapes the rest of the guard wants:
 *
 *   mask      -> string[] of field names, unchanged from what it always was
 *   maskRules -> {field: {type, ...}} for the entries that ask for more than a placeholder
 *
 * Splitting them keeps `Decision.fieldsToMask` a plain list of names, which is what the audit log
 * line and the OTel span attribute already publish — a rule object in there would change the shape
 * of both for every consumer, to say something none of them asked about.
 *
 * An entry is either a bare field name (masked to the placeholder, the original behaviour) or an
 * object naming a strategy:
 *
 *   { "field": "iban",  "type": "partial", "keepLeft": 4, "keepRight": 4, "char": "*" }
 *   { "field": "email", "type": "email" }
 *
 * `keepLeft`/`keepRight` only mean anything for "partial" and `char` only for the two strategies
 * that draw with it, so supplying them elsewhere is rejected rather than ignored: a config that
 * asks for something it will not get should say so at load time.
 */
function normalizeMask(entityName, rawMask) {
  const mask = [];
  const rules = {};

  for (const entry of rawMask) {
    if (typeof entry === 'string') {
      if (!entry) throw new Error(`entities.${entityName}.mask entries must be non-empty strings`);
      mask.push(entry);
      continue;
    }
    ...
    if (!entry || typeof entry !== 'object' || Array.isArray(entry)) {
      throw new Error(
        `entities.${entityName}.mask entries must be a string or a {field, type} object, got ${typeOf(entry)}`
      );
    }

    const { field, type = 'full', keepLeft, keepRight, char } = entry;

    if (typeof field !== 'string' || !field) {
      throw new Error(`entities.${entityName}.mask entries must have a "field" string`);
    }
    if (!MASK_TYPES.includes(type)) {
      throw new Error(
        `entities.${entityName}.mask.${field}.type must be one of ${MASK_TYPES.join(', ')} (got ${JSON.stringify(type)})`
      );
    }

    for (const [key, value] of [['keepLeft', keepLeft], ['keepRight', keepRight]]) {
      if (value === undefined) continue;
      if (type !== 'partial') {
        throw new Error(`entities.${entityName}.mask.${field}: "${key}" is only allowed with type "partial"`);
      }
      if (!Number.isInteger(value) || value < 0) {
        throw new Error(
          `entities.${entityName}.mask.${field}.${key} must be a non-negative integer, got ${JSON.stringify(value)}`
        );
      }
    }
    ...
    if (char !== undefined) {
      if (type === 'full') {
        throw new Error(
          `entities.${entityName}.mask.${field}: "char" is only allowed with type "partial" or "email" ` +
            '(use "maskValue" to change the placeholder itself)'
        );
      }
      if (typeof char !== 'string' || [...char].length !== 1) {
        throw new Error(
          `entities.${entityName}.mask.${field}.char must be a single character, got ${JSON.stringify(char)}`
        );
      }
    }
    ...
    if (type !== 'full') {
      rules[field] = {
        type,
        ...(keepLeft !== undefined && { keepLeft }),
        ...(keepRight !== undefined && { keepRight }),
        ...(char !== undefined && { char })
      };
    }
  }

  return { mask, ...(Object.keys(rules).length > 0 && { maskRules: rules }) };
}
...
function validateEntityConfig(name, raw) {
  ...
  const { mask: rawMask, maxRows, allowTools, pseudonymize: rawPseudonymize } = config;

  if (rawMask !== undefined && !Array.isArray(rawMask)) {
    throw new Error(`entities.${name}.mask must be an array, got ${typeOf(rawMask)}`);
  }

  const { mask, maskRules } = rawMask === undefined ? {} : normalizeMask(name, rawMask);
  ...
  return { mask, maxRows, allowTools, pseudonymize, ...(maskRules !== undefined && { maskRules }) };
}
...
function parseConfig(raw, opts = {}) {
  ...
  if (raw.paths !== undefined) {
    if (!Array.isArray(raw.paths)) {
      throw new Error(`"paths" must be an array, got ${typeOf(raw.paths)}`);
    }
    for (const entry of raw.paths) {
      // Anchored at the root, because these are matched against the inbound path as a prefix and a
      // bare "mcp" would match nothing — a config that silently scopes the policy to no request at
      // all is the one mistake here that looks exactly like a working one.
      if (typeof entry !== 'string' || !entry.startsWith('/')) {
        throw new Error(
          `"paths" entries must be strings starting with "/" (got ${JSON.stringify(entry)})`
        );
      }
    }
  }
  ...
  if (raw.otherSurfacesGated !== undefined && typeof raw.otherSurfacesGated !== 'boolean') {
    throw new Error(`"otherSurfacesGated" must be a boolean, got ${typeOf(raw.otherSurfacesGated)}`);
  }

  if (raw.whenPathUnknown !== undefined && !WHEN_PATH_UNKNOWN.includes(raw.whenPathUnknown)) {
    throw new Error(
      `"whenPathUnknown" must be one of ${WHEN_PATH_UNKNOWN.join(', ')} (got ${JSON.stringify(raw.whenPathUnknown)})`
    );
  }
  if (raw.whenPathUnknown !== undefined && raw.paths === undefined) {
    throw new Error('"whenPathUnknown" only applies with "paths" set');
  }
  ...
  return {
    ...
    ...(raw.whenPathUnknown !== undefined && { whenPathUnknown: raw.whenPathUnknown }),
    ...(raw.otherSurfacesGated !== undefined && { otherSurfacesGated: raw.otherSurfacesGated }),
    audit,
